...
export default function ContactCard({
  ...
  const Wrapper = ({ children }: { children: React.ReactNode }) => {
    if (href) {
      return (
        <a
          href={href}
          className="w-full h-full block focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 focus-visible:ring-offset-2 rounded-xl"
    ...
    return <div className="w-full h-full block rounded-xl">{children}</div>;
  };
  ...
  return (
    <motion.div
      ...
      whileTap={cardTap}
      className={`w-full ${className} max-h-[150px]`}
      style={{ touchAction: "manipulation" }}
    >
      <Wrapper> 
        <div className="bg-gray-800 rounded-xl p-6 transition-all hover:-translate-y-1 hover:shadow-2xl hover:bg-gray-800/90 hover:border-emerald-800/50 border border-gray-700/50 duration-300 w-full h-full">
